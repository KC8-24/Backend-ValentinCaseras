...
router.get("/:pid", async (req, res) => {
    try {
        ...
        if (!product) {
            return res.status(404).json({success: false, error: "No se escontro el encontrado"})
        } else {
            res.status(200).json({success: true, data: product})
        }
    } catch (error) {
        console.log(error)
        res.status(500).json({success: false, error: "No se escontro el encontrado"})
    }
})
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!title || !description || !code || !price || !stock || !category) {
            return res.status(400).send("Algunos de los datos no han sido completados")
        }
